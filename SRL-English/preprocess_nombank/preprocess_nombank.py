...
root_pathp = FileSystemPathPointer('./datasets/nombank.1.0')
...
def run():
    ...
    train_filepath = './datasets/preprocess_nombank/train.srl'
    devel_filepath = './datasets/preprocess_nombank/development.srl'
    test_filepath = './datasets/preprocess_nombank/test.srl'
...
def count_nombank_samples():
    for split in ['train', 'development', 'test']:
        path = f'./datasets/preprocess_nombank/{split}.srl'
        seen_sentences = list()
        counts = 0
        with open(path, 'r') as f:
            data = f.readlines()
        for line in data:
            tag = '_'.join(line.split()[:2])
            if tag not in seen_sentences:
                seen_sentences.append(tag)
                counts += 1
...
def count_ontonotes_samples():
    root = './datasets/conll-formatted-ontonotes-5.0-12/conll-formatted-ontonotes-5.0/data/{}/data/english/annotations'
    for split in ['train', 'development', 'test']:
        sent_counts = 0
        folder = root.format(split)
        for bc in os.listdir(folder):
            word_counts = 0
            for cctv in os.listdir(os.path.join(folder, bc)):
                for index in os.listdir(os.path.join(folder, bc, cctv)):
                    cur_path = os.path.join(folder, bc, cctv, index)
                    for file in os.listdir(cur_path):
                        if file.endswith('.gold_skel'):
                            continue
                        ...
                        for line in data:
                            if line.startswith('#') or line == '\n':
                                continue
                            if not line.startswith(f'{bc}/{cctv}/{index}'):
                                print(line)
                                return
                            doc_id, part_id, word_id = line.split()[:3]
                            if word_id == '0':
                                sent_counts += 1
                            word_counts += 1
            print(f'{split}: {bc}, words: {word_counts}')
        print(f'{split}: sentences: {sent_counts}')
    """
    train: sentences: 115812
    development: sentences: 15680
    test: sentences: 12217
    """
    return


if __name__ == '__main__':
    # count_nombank_samples()
    count_ontonotes_samples()