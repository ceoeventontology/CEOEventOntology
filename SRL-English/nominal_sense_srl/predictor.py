...
class NomSenseSRLPredictor(Predictor):
    ...
    def tokens_to_instances(self, tokens, indices, separate_hyphens_flag):
        """
        # Parameters
        ...
        if separate_hyphens_flag:
            ...
            new_sentence, new_indices = deepcopy(words), [[i] for i in range(len(words))]
    ...
    def _sentence_to_srl_instances(self, json_dict: JsonDict) -> List[Instance]:
        ...
        if 'tokens' in json_dict:
            ...
            output = self.tokens_to_instances(tokens, indices, separate_hyphens_flag=False)
        else:
            sentence = json_dict["sentence"]
            tokens = self._tokenizer.split_words(sentence)
            output = self.tokens_to_instances(tokens, indices, separate_hyphens_flag=True)
        return output
            ...
            results["nominals"].append(
                    {
                        ...
                        'nominal_indices': output['nominal_indices'],
                        "sense": output["sense"],
                        "predicate_index": output["nominal_indices"],
    ...
    def predict_json(self, inputs: JsonDict) -> JsonDict:
        ...
        if not instances:
            if 'tokens' in inputs:
                output = {"nominals": [], "words": self._tokenizer._sanitize([token for token in self.nlp(inputs['tokens'])])}
